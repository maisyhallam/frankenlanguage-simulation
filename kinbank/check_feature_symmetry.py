...
def write_full_csv(filename,file,full_results,feature):
    language = get_language_name(file)
    if feature == 'lineage':
    ...
    with open(filename, 'a', encoding='utf8') as csv_file:
        csvwriter = csv.writer(csv_file)
        csvwriter.writerow([language,generation,result])
...
with open('gender_distinction.csv', 'a') as csv_file:
    csvwriter = csv.writer(csv_file)
    # csvwriter.writerow(['LANGUAGE','GENDER SYMMETRY','AGE SYMMETRY','LINEAGE SYMMETRY'])
    csvwriter.writerow(['LANGUAGE','GENERATION','GENDER'])
...
for family in families:
    ...
    for file in files:
        ...
        for i in feature_symmetry_results:
            full_results[i] = feature_symmetry_results[i]
            # problem is with the above line, doesn't append, just changes to the current file
    write_full_csv('gender_distinction.csv',file,feature_symmetry_results,'gender')
