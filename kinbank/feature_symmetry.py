...
def check_feature_symmetry(family,file):
    ...
    kin_types = ['siblings','mz cousins','mb cousins','fz cousins','fb cousins','m siblings', 'f siblings']
    generations = ['G0','G0','G0','G0','G0','G+1','G+1']

    gender_pairs = [[('meB','meZ'),('myB','myZ')], #siblings
    [('mMZeS','mMZeD'),('mMZyS','mMZeD')], #mz cousins
    [('mMBeS','mMBeD'),('mMByS','mMBeD')], #mb cousins
    [('mFZeS','mFZeD'),('mFZyS','mFZeD')], #fz cousins
    [('mFBeS','mFBeD'),('mFByS','mFBeD')], #fb cousins
    ...
    for i in range(len(kin_types)):
        ...
        try:
            ...
            age_result = check_equality(age[i][0],age[i][1])
            results.append((kin_types[i],generations[i],gender_result,age_result))
        except:
            continue
...
def write_to_csv(filename,family,file):
    ...
    with open(filename, 'a', encoding="utf8") as csv_file:
        ...
        for kin_type,generation,gender,age in line:
            csvwriter.writerow([language,kin_type,generation,gender,age])
...
with open('feature_symmetry.csv', 'a') as csv_file:
    csvwriter = csv.writer(csv_file)
    csvwriter.writerow(['LANGUAGE','KIN TYPE','GENERATION','GENDER DISTINGUISHED','AGE DISTINGUISHED'])
